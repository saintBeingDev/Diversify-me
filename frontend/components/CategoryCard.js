...
const CategoryCard = ({filteredPosts}) => {
  ...
  useEffect(() => {
    ...
    try {
      if(slug){
        ...
        let posts = categoryPost.filter(p=>( p.title.trim().toLowerCase().indexOf(value)) !== -1 || // for searching with title
        (p.categories.map(cat=>(cat.title.trim().toLowerCase().indexOf(value)))) >=0  || // for searching with category
  ...
  return (
    <>
       {
        loading ? <div className="md:col-span-full min-h-screen flex justify-center items-center">
          <Loader/>
        </div>  :
        <>
    ...
    {categoryPost.length === 0 ? 
       ...
       <h1 className="flex items-center justify-center text-3xl text-gray-400">No blog found</h1>
       </div>
       ...
       <>{categoryPost.map((post) => (
          <>
          <div key={post.title} className="flex flex-col sm:col-span-3 md:col-span-1 justify-between shadow-lg dark:bg-darkSecondary bg-reactionIconBg h-fit md:h-[420px] mt-2 p-3 w-full rounded-md">
            ...
            {post.categories.map((cat) => (
              <h6 key={cat.title} className="mt-1.5 rounded-lg px-2 py-1 bg-green-200 text-green-700 dark:bg-secondary dark:text-gray-300 text-xs w-fit">
                {cat.title}
              </h6>
            ))}
            </div>
